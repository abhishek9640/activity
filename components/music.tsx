...
const Music: React.FC = () => {
    ...
    const fetchActivities = async () => {
      try {
        const responses = await Promise.all([
          axios.get('https://www.boredapi.com/api/activity?type=music&participants=1'),
          axios.get('https://www.boredapi.com/api/activity?type=music&participants=1'),
          axios.get('https://www.boredapi.com/api/activity?type=music&participants=1')
        ]);
  ...
  if (activeSection !== links[3].name) {
    return null
  }

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Music Activities</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {activities.map((activity, index) => (
          <motion.div
            ...
            className={`max-w-[42rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] transition sm:group-even:pl-8 ${index % 2 === 0 ? 'bg-[#596e45]' : 'bg-[#532632]'}`}
          >
